...
WORDLIST_FILENAME = "C:/dev/6.00.1x Files/words.txt"
...
def getAvailableLetters(lettersGuessed):
    ...
    for ch in lettersGuessed:
        avaiLetters = avaiLetters.replace(ch, '')
    return avaiLetters
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while count > 0:
        ...
        if (secretWord == getGuessedWord(secretWord, list)):
            print('Congratulations, you won!')
            break
            
        print('You have ' + str(count) + ' guesses left.')
        availLettters = getAvailableLetters(list)
        print('Available letters: ' + availLettters)
        ...
        if (guess in list):
            ...
            list.append(guess)        
            guessedWord = getGuessedWord(secretWord, list)
